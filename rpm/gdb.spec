...
# For our convenience
%global gdb_src gdb-%{version}/upstream
# FIXME: VPATH?
...
# Scratchbox2 support
Patch4001: 0001-Add-support-for-Scratchbox2-to-take-executing-with-l.patch 
...
%define supported_32bit_arch x86_64
...
(
 mkdir -p processor-trace-%{libipt_version}-root
 mkdir -p processor-trace-%{libipt_version}-build
...
%define extra_target_list_common i686 x86_64 aarch64 armv7l
%if 0%{?is_opensuse}
...
# Because we build from the same source we have to disable the programs
# which would not be in the source tarball.
disabled_programs="--disable-binutils \
        --disable-ld \
        --disable-gold \
        --disable-gas \
        --disable-gprof \
...
cp -p $RPM_BUILD_DIR/%{gdb_src}/%{gdb_build}/gdb/gdb-gdb.py $RPM_BUILD_ROOT/usr/lib/debug%{_bindir}/
for pyo in "" "-O";do
